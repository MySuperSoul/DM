...
- (iii) The minimum total risk $(R = \sum_xmin_iR(\alpha_i \mid x) = \sum_xmin_i \lambda(\alpha_i \mid \omega_j) \cdot p(\omega_j \mid x))$, here we know the loss matrix $\lambda(\alpha_i \mid \omega_j)$, and I take the training set posterior into the formula, I calculate the result is **2.444354**, the codes shown below, also can see in the code folder:
```matlab
% p is the posterior
risk = [0, 1; 2, 0];
...
<br>
- **(b):** $Posterior = p(y_i\mid \mathtt{x}) = \frac{p(\mathtt{x} \mid y_i)p(y_i)}{p(\mathtt{x})}$, in this task already know the likelihood formula form and $prior = p(y_i) = \phi_i, p(\mathtt{x}) = \sum_{i=1}^mp(\mathtt{x}\mid y_i)p(y_i)$, so according to the information we know above, the implementation below, you can also find these in code folder and some parameter description:
```matlab
%   Inputs:
...
- **(b):**
The accuracy rate of my classifier on test set is **98.573156%.**, you can check this simply by running the `run.m` in the `text_classification` code folder.
<br>
